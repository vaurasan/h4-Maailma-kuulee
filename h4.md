...
- Nyt voidaan sulkea *root*-käyttäjä. ``sudo usermod --lock root`` komennolla saadaan root-salasana pois. Nyt mennään ``sudoedit /etc/ssh/sshd_config``, ja muutetaan ``PermitRootLogin no``. Näin saadaan root lukittua.
